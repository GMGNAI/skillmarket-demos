...
- **dev evaluation dimension** (`dev_score`, pure-code 0..1; **both a ranking sub-score and a filter gate**, implements the demo's real algorithm):
  ...
  - **reskin `reskin`**: `image_dup_count` (the dev reuses the same image across many new tokens = relaunching the same playbook; the demo's "换皮重发"; normal tokens unaffected). ⚠️ **`twitter_name_change_history` is NOT used**: a token's twitter handle is freely set by the project as a concept association, not a verified dev identity, so using it to judge "reskin" causes false positives (user correction). **exited** mild penalty; `cto_flag` small positive.
...
- `dev` (DevProfile dict): `creator` (dev wallet address), `analyzed`/`alive`/`rugged`/`rug_rate` (per-token classification: analyzed count / alive / rugged / rug rate), `launches` (open_count, graduated), `inner_count` (stuck-inner), `survival_rate` (open_ratio), `sec_checked`/`sec_unsafe`/`sec_risks`/`sec_risk_rate` (security scan: how many checked / how many unsafe / risk labels / unsafe rate), `ath_mc`, `exited`, `image_dup` (same-image reuse = reskin signal), `cto`. Live = `token info` (creator/reskin/exited) + `portfolio created-tokens` (launch history, per-token classification by `_merge_created`) + `token security` ×N (`_scan_dev_security`); Mock synthesizes an isomorphic one. `_dev_reskin(dp)` derives reskin strength from `image_dup`; `_security_unsafe(sec, chain)` classifies a single token's safety per chain.

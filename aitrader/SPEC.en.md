...
- **dev evaluation dimension** (`dev_score`, pure-code 0..1 bonus term; uses only stable-semantics signals, per the "only use stable signals" discipline): the main score is the peak market cap of the dev's best prior token `ath_mc` (has the dev ever made a golden runner); penalties are serial launching `creator_open_count`>50 (token-factory risk) and having exited this token `creator_token_status`+balance (misaligned incentives); `cto_flag` (community takeover) is a small positive. `twitter_del_post_token_count` is commonly a large noisy number with unstable semantics → **not scored**, only exposed in `_feat` for reference. Data comes from the `dev` object in `token info` — **an extra cli call is made only for the top `dev_pool_n`(24) survivors after initial ranking**, results cached per-address for `dev_info_ttl_s`(600s) and reused across cycles rather than re-fetched. Not found → neutral 0.5, no false kill, no blocking.
...
- **New dev evaluation dimension** (see §4): the `dev` object from `token info` → `dev_score` (past golden runner adds / serial launching · exited-this-token subtract) as a ranking sub-score folded into `priority_score`; two-pass ranking (initial sort → query dev history only for the top 24 → re-rank), results cached per-address for 600s to save quota; Mock synthesizes an isomorphic profile so it runs with no key. **New frontend `DEV REP` column** (GOOD/OK/WEAK + 0–100 score, hover shows best prior token mcap / launch count / whether exited + expand detail + full DEMO seed data).

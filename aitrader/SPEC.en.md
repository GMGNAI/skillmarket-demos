...
trending (cheap, 1 cli call, the row already contains all due-diligence fields)
  ...
  → dev evaluation dimension (query dev wallet launch history via portfolio created-tokens for the top dev_pool_n only, TTL-cached) → fold dev sub-score into priority_score, re-rank + filter out low scores (token factories / reskinners)
...
- **dev evaluation dimension** (`dev_score`, pure-code 0..1; **both a ranking sub-score and a filter gate**, implements the demo's real algorithm):
  - **Data source = `portfolio created-tokens` (dev wallet launch history)**: first `token info` gives `dev.creator_address`, then `portfolio created-tokens --wallet <creator>` returns the dev's full launch list + per-token status (`is_open`/`liquidity_less_4k`/`market_cap`) in one call, with top-level `inner_count` (total sprayed), `open_count` (migrated launches), `open_ratio` (**survival/migration rate**), `creator_ath_info`.
  - **main score = survival rate `survival_rate`(open_ratio)**: the fraction of the dev's launches that survived. 100% → good, ~1% → token factory (this is the real source for the demo's "rug rate / survival count").
  - **token-factory penalty `inner_count`**: mass-spraying bonding-curve tokens (often thousands) = batch dumper (50→0, 1000→max, -0.35).
  ...
  - Real calibration: the LMAO! dev (10659 sprayed · 124 migrated · **1.1% survival** · 10 renames · exited · rugged at 4 min) → v3 scores **0.0** and is killed by the filter gate; a clean dev (3 launches all alive · one hit $5M) → 0.93 GOOD.
...
| `portfolio created-tokens` | dev eval | query the dev wallet's launch history (spray count / survival rate / per-token rug status) → main dev_score source | top dev_pool_n survivors per cycle (600s cached) |
...
- **Dev reputation card** (click a token row → bottom-right detail): replicates the demo's "reputation score 0–100 + launched / rugged (rate) / alive / reskin + trusted/untrusted", sourced from `portfolio created-tokens`.
...
- **dev evaluation dimension v3** (see §4): data source switched to **`portfolio created-tokens` (dev wallet launch history)** (implements the demo's real algorithm) — **survival rate (open_ratio) driven** + token-factory (inner_count) / reskin / exited penalties. Acts as **both a ranking sub-score** (folded into `priority_score`) **and a filter gate** (`min_dev_score` kills token factories; a "Low dev reputation" kill appears in the scan stream). **Frontend**: `DEV REP` column (GOOD/OK/WEAK + 0–100 + red reskin tag) + **clicking a token row pops a Dev reputation card bottom-right** (replicates the demo: score / launched / rugged-rate / alive / reskin / trusted). Real calibration: the LMAO! dev (1.1% survival · 10659 sprayed) → 0.0, filtered out; Mock synthesizes an isomorphic profile so it runs with no key.
...
- `dev` (DevProfile dict): `creator` (dev wallet address), `launches` (open_count, migrated launches), `inner_count` (total sprayed), `survival_rate` (open_ratio), `alive`/`rugged`/`rug_rate` (display, derived from launches×survival), `ath_mc` (best prior token peak), `exited`, `name_changes`/`image_dup` (reskin signals), `del_post_count`, `cto`. Live = `token info` (creator/reskin/exited) + `portfolio created-tokens` (launch history/survival, merged by `_merge_created`); Mock synthesizes an isomorphic one. `_dev_reskin(dp)` derives the reskin strength (0..1) from `name_changes`/`image_dup`.

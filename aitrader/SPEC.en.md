...
- **dev evaluation dimension** (`dev_score`, pure-code 0..1 bonus term; hit-rate / survival-rate thinking, inspired by a "dev reputation score" design):
  - **track record `track`**: peak market cap of the dev's best prior token `ath_mc`, but **discounted by launch count** — one runner amortized over hundreds of launches = very low hit rate / low signal (heavy serial launching cuts the track record to ~1/3).
  - **serial launching `serial`**: `creator_open_count` (≤20 is normal, 200 = max) — higher looks more like a token factory; subtracts directly and discounts the track record.
  - **reskin re-launch `reskin`**: `twitter_name_change_history` (repeated identity changes) + `image_dup_count` (reused image) → serial reskin scam (the demo's "换皮重发"; normal tokens with name_changes≤2 / image_dup≤1 are unaffected).
  - **exited this token `exited`**: `creator_token_status`+balance; a serial launcher who has already exited = pre-positioned to dump, compounded with `serial`.
  - `cto_flag` (community takeover) is a small positive; `twitter_del_post_token_count` has unstable semantics → not scored (only exposed in `_feat`).
  - ⚠️ **Data gap**: the demo's headline signals **rug count/rate and surviving count** are **not** directly available from gmgn-cli (would require enumerating each of the dev's historical tokens and classifying rugs — blows the cli budget), so the above (serial + reskin + exited + discounted track) are used as proxies.
  - Data comes from the `dev` object in `token info` — **an extra cli call is made only for the top `dev_pool_n`(24) survivors after initial ranking**, results cached per-address for `dev_info_ttl_s`(600s) and reused across cycles. Not found → neutral 0.5, no false kill, no blocking.
  - Calibrated on a real failure case: LMAO! (124 launches · 10 renames · 11 dup images · exited · rugged at 4 min) was misjudged at 0.73 GOOD by the old algorithm → v2 corrects it to 0.12 WEAK + reskin detected. But dev is a **ranking sub-score with no veto**: it down-ranks such tokens and shows a red frontend warning, it does **not** block the buy (a 4-min rug is a real-time liquidity-pull event, the job of the escape monitor / gate1, not a reputation score).
...
> Note `DEV` (holding %) and `DEV REP` (dev reputation) are two different columns: the former is the dev's current holding ratio (gate1 rug gate), the latter is the new dev-evaluation ranking sub-score (see §4). `DEV REP` only has a value for the few survivors whose dev history was queried this cycle (top dev_pool_n); tokens cut at gate1/2 or outside the ranking pool show `—`.
...
- **New dev evaluation dimension** (see §4): the `dev` object from `token info` → `dev_score` (hit-rate thinking: best-ever mcap discounted by launch count + serial-launching / reskin-relaunch / exited subtract) as a ranking sub-score folded into `priority_score`; two-pass ranking (initial sort → query dev history only for the top 24 → re-rank), results cached per-address for 600s to save quota; Mock synthesizes an isomorphic profile so it runs with no key. **New frontend `DEV REP` column** (GOOD/OK/WEAK + 0–100 score + red reskin tag, hover shows best prior token mcap / launch count / exited / reskin + expand detail + full DEMO seed data). Calibrated on a real rug case (LMAO! rugged at 4 min): v2 corrects it from 0.73 GOOD to 0.12 WEAK + reskin detected.
...
- `dev` (DevProfile dict): `open_count` (creator_open_count, lifetime tokens launched), `status`/`balance` (creator_token_status/balance), `exited` (already sold out of this token), `ath_mc` (peak market cap of dev's best prior token), `name_changes` (twitter_name_change_history length), `image_dup` (image_dup_count, tokens reusing the same image), `del_post_count` (deleted promo tweets), `create_count`, `cto`. Live reads it from the `dev` object in `token info`; Mock synthesizes an isomorphic one. `_dev_reskin(dp)` derives the reskin-relaunch strength (0..1) from `name_changes`/`image_dup`.

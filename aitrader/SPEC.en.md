...
Screening result table columns: TOKEN (clickable to copy the ticker + radar icon = already holding this coin) / rule→rank→LLM (gate icons) / safety (honeypot · authority-renounce badge) / BUND / DEV / T10 / smart money/KOL (degen/kol) / **DEV REP (dev reputation score GOOD/OK/WEAK + 0–100; hover shows best prior token mcap / launch count / whether the dev exited; shows — when not evaluated)** / timing (early · sideways · overheated · bleeding) / LLM (pass/watch/reject) / priority / decision.
> Note `DEV` (holding %) and `DEV REP` (dev reputation) are two different columns: the former is the dev's current holding ratio (gate1 rug gate), the latter is the dev evaluation (see §4) — both a ranking sub-score and a filter gate. `DEV REP` only has a value for the few survivors whose dev history was queried this cycle (top dev_pool_n); tokens cut at gate1/2 or outside the ranking pool show `—`; tokens with too-low dev scores are killed by the filter gate (a "Low dev reputation" kill reason appears in the scan stream).

...
trending (cheap, 1 cli call, the row already contains all due-diligence fields)
  ...
  → dev evaluation dimension (query dev wallet launch history via portfolio created-tokens for the top dev_pool_n only, TTL-cached) → fold dev sub-score (graduation-rate driven) into priority_score, re-rank + filter out low scores (token factories / stuck-inner / reskinners)
...
- **dev evaluation dimension** (`dev_score`, pure-code 0..1; **both a ranking sub-score and a filter gate**, implements the demo's real algorithm):
  ...
  - **main score = graduation rate `survival_rate`(open_ratio)**: the fraction of the dev's tokens that graduated to an outer pool. 100% → good, ~1% → token factory (this is the real source for the demo's "survival / rug").
  - **stuck-inner penalty `inner_count`**: tons of tokens stuck in the inner pool that never graduated (often thousands) = batch token factory (50→0, 1000→max, -0.35).
  - **track record `ath_mc`** adds a little, but **gated by graduation rate** (a factory's one runner is luck, not counted).
  ...
  - Cost: each evaluated dev = `token info` + `created-tokens` (two cli), **only for the top `dev_pool_n`(24) survivors**, cached per-address for `dev_info_ttl_s`(600s).
...
- **Dev reputation card** (click a token row → bottom-right detail): reputation score 0–100 + graduated / stuck-inner / graduation rate / reskin + trusted/untrusted, sourced from `portfolio created-tokens`.
...
- Dev eval: `dev_pool_n=24` (after initial ranking, query dev history for the top N; >llm_max so dev can re-rank the gate3 boundary), `dev_info_ttl_s=600` (per-address dev-history cache seconds), `min_dev_score=0.15` (dev filter gate: token factories / serial reskinners below this are killed).
...
- **dev evaluation dimension v3** (see §4): data source switched to **`portfolio created-tokens` (dev wallet launch history)** (implements the demo's real algorithm) — **graduation rate (open_ratio) driven** + stuck-inner (inner_count) / reskin / exited penalties. Acts as **both a ranking sub-score** (folded into `priority_score`) **and a filter gate** (`min_dev_score` kills token factories; a "Low dev reputation" kill appears in the scan stream). **Frontend**: `DEV REP` column (GOOD/OK/WEAK + 0–100 + red reskin tag) + **clicking a token row pops a Dev reputation card bottom-right** (graduated / stuck-inner / graduation rate / reskin / trusted). Real calibration: the LMAO! dev (1.1% graduation · 10659 stuck inner) → 0.0, filtered out; Mock synthesizes an isomorphic profile so it runs with no key.
...
- `dev` (DevProfile dict): `creator` (dev wallet address), `launches` (open_count, graduated to outer pool), `inner_count` (stuck-inner: tokens stuck in the bonding curve that never graduated), `survival_rate` (open_ratio, graduation rate), `ath_mc` (best prior token peak), `exited`, `image_dup` (same-image reuse = reskin signal), `del_post_count`, `cto`. Live = `token info` (creator/reskin/exited) + `portfolio created-tokens` (launch history/graduation rate, merged by `_merge_created`); Mock synthesizes an isomorphic one. `_dev_reskin(dp)` derives the reskin strength (0..1) from `name_changes`/`image_dup`.

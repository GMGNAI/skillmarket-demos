...
- **dev evaluation dimension** (`dev_score`, pure-code 0..1; **both a ranking sub-score and a filter gate**, implements the demo's real algorithm):
  ...
  - **reskin `reskin`**: `twitter_name_change_history` + `image_dup_count` (the demo's "换皮重发"; normal tokens unaffected); **exited** mild penalty; `cto_flag` small positive.
...
- `dev` (DevProfile dict): `creator` (dev wallet address), `launches` (open_count, graduated to outer pool), `inner_count` (stuck-inner: tokens stuck in the bonding curve that never graduated), `survival_rate` (open_ratio, graduation rate), `ath_mc` (best prior token peak), `exited`, `name_changes`/`image_dup` (reskin signals), `del_post_count`, `cto`. Live = `token info` (creator/reskin/exited) + `portfolio created-tokens` (launch history/graduation rate, merged by `_merge_created`); Mock synthesizes an isomorphic one. `_dev_reskin(dp)` derives the reskin strength (0..1) from `name_changes`/`image_dup`.

...
trending (cheap, 1 cli call, the row already contains all due-diligence fields)
  ...
  → dev evaluation dimension (query dev wallet launch history via portfolio created-tokens + per-token token-security scan of recent launches, top dev_pool_n only, TTL-cached) → fold dev sub-score (survival-rate + security driven) into priority_score, re-rank + filter out low scores (token factories / unsafe launches / reskinners)
...
- **dev evaluation dimension** (`dev_score`, pure-code 0..1; **both a ranking sub-score and a filter gate**, implements the demo's real algorithm):
  - **Data source = `portfolio created-tokens` (dev wallet launch history)**: first `token info` gives `dev.creator_address`, then `portfolio created-tokens --wallet <creator>` returns the dev's full launch list + per-token status (`is_open`/`liquidity_less_4k`/`market_cap`) in one call, with top-level `inner_count` (**stuck-inner**: tokens stuck in the bonding curve inner pool that never filled to graduate), `open_count` (**graduated**: filled the curve and migrated to an outer pool), `open_ratio` (**graduation rate** = graduated /(graduated + stuck-inner)), `creator_ath_info`.
  - **main score = survival rate (per-token classification)**: each token in created-tokens is classified alive vs rugged by `is_open + liquidity_less_4k`; survival = alive/analyzed (= 1 − rug rate). 100% → good, ~1% → token factory (this is the source for the demo card's "launched / rugged / alive").
  - **per-token security scan `sec_risk_rate`** (user request): for the dev's **most recent `dev_sec_scan_n`(3) launches**, query `token security` and flag unsafe per chain — **Sol**: mintable (mint not renounced) / freeze not renounced / honeypot; **EVM**: not open-source / can't-sell (honeypot). The higher the unsafe fraction, the lower the score, and the risk is surfaced on the card + reject reason.
  - **stuck-inner penalty `inner_count`**: tons of tokens stuck in the inner pool that never graduated (often thousands) = batch token factory (50→0, 1000→max, -0.30).
  - **track record `ath_mc`** adds a little, but **gated by survival rate** (a factory's one runner is luck, not counted).
  - **reskin `reskin`**: only counts the logo reuse **across this dev's OWN launches** (`own_img_reuse` = duplicate-logo count among the dev's created-tokens; 1 self-reuse tolerated, ≥2 flagged). ⚠️ **the global `image_dup_count` is NOT used**: others stealing the image to launch new tokens inflate the global count and falsely punish the original creator who launched only 1 token (user correction: the `FeMbDo…` dev launched 1 token but its image was stolen 10×, which the old algorithm misjudged as max reskin). **`twitter_name_change_history` is also not used** (the handle is freely set by the project, not a verified dev identity). **exited** mild penalty; `cto_flag` small positive.
  - **filter gate**: `dev_score < min_dev_score`(0.15) → killed (reason e.g. "Low dev reputation (0/100: 10659 stuck inner · 1% graduation · reskin · exited)"), matching the demo's scan-stream "⛔ Dev serial rug". ⚠️ This is a deliberate relaxation (user request) of the original "dev only ranks, never vetoes" rule: dev can now block token factories / serial reskinners, not just down-rank.
  - Fallback: if `created-tokens` is unavailable (no survival_rate) → degrade to `open_count`+`ath` discounted track, no blocking.
  - Cost: each evaluated dev = `token info` + `created-tokens` + `token security` ×`dev_sec_scan_n`(3) ≈ 5 cli, **only for the top `dev_pool_n`(24) survivors**; the whole dev profile (including the security scan) is cached per-address for `dev_info_ttl_s`(600s), not re-scanned every cycle.
  - Real calibration: the LMAO! dev (10659 stuck inner · 124 graduated · **1.1% graduation** · 10 renames · exited · rugged at 4 min) → v3 scores **0.0** and is killed by the filter gate; a clean dev (3 launches all graduated · one hit $5M) → 0.93 GOOD.
...
| `token security` | escape / dev sec scan | normalized safety snapshot; on-board positions reuse the trending row; **dev eval scans the dev's most recent dev_sec_scan_n launches (mintable / not-renounced / not-open-source / honeypot)** | off-board positions + dev sec scan (600s cached) |
...
| `portfolio created-tokens` | dev eval | query the dev wallet's launch history (stuck-inner count / graduation rate / per-token status) → main dev_score source | top dev_pool_n survivors per cycle (600s cached) |
...
Screening result table columns: TOKEN (clickable to copy the ticker + radar icon = already holding this coin) / rule→rank→LLM (gate icons) / safety (honeypot · authority-renounce badge) / BUND / DEV / T10 / smart money/KOL (degen/kol) / **DEV Score (dev reputation GOOD/OK/WEAK + 0–100 + red reskin tag; hover shows launched / rug rate / alive / reskin; shows — when not evaluated)** / timing (early · sideways · overheated · bleeding) / LLM (pass/watch/reject) / priority / decision.
> Column headers are localized: EN "BUND/DEV/T10", ZH "捆绑/DEV/前10".
> Note `DEV` (dev's current holding %) and `DEV Score` (dev reputation) are two different columns: the former is the dev's holding ratio (gate1 rug gate), the latter is the dev evaluation (see §4) — both a ranking sub-score and a filter gate. `DEV Score` only has a value for the few survivors whose dev history was queried this cycle (top dev_pool_n); tokens cut at gate1/2 or outside the ranking pool show `—`; tokens with too-low dev scores are killed by the filter gate (a "Low dev reputation" kill reason appears in the scan stream).
- **Dev reputation card** (click a token row → bottom-right detail): reputation score 0–100 + **launched / rugged (rate) / alive / reskin** + trusted/untrusted + a security-scan risk line (shown when recent launches are unsafe: "⚠ M/N recent launches unsafe: mintable/honeypot…"), sourced from `portfolio created-tokens` + per-token `token security`.
...
- Dev eval: `dev_pool_n=24` (after initial ranking, query dev history for the top N; >llm_max so dev can re-rank the gate3 boundary), `dev_info_ttl_s=600` (per-address dev-profile cache seconds), `min_dev_score=0.15` (dev filter gate: token factories / serial reskinners below this are killed), `dev_sec_scan_n=3` (scan the dev's most recent N launches via token security).
...
- **dev evaluation dimension v3** (see §4): data source = **`portfolio created-tokens` (dev wallet launch history) + per-token `token security` scan of the most recent N launches** (implements the demo's real algorithm) — **per-token survival-rate driven** + unsafe-launch (mintable/not-renounced/not-open-source/honeypot) / stuck-inner / reskin / exited penalties. Acts as **both a ranking sub-score and a filter gate** (`min_dev_score` kills token factories; a "Low dev reputation" kill appears in the scan stream). **Frontend**: `DEV Score` column + **clicking a token row pops a Dev reputation card bottom-right** (score + launched / rugged (rate) / alive / reskin + a security-scan risk line + trusted). Real calibration: the LMAO! dev (98% rug · 10659 stuck inner) → 0.0, filtered out; Mock synthesizes an isomorphic profile so it runs with no key.
...
- `dev` (DevProfile dict): `creator` (dev wallet address), `analyzed`/`alive`/`rugged`/`rug_rate` (per-token classification: analyzed count / alive / rugged / rug rate), `launches` (open_count, graduated), `inner_count` (stuck-inner), `survival_rate` (open_ratio), `sec_checked`/`sec_unsafe`/`sec_risks`/`sec_risk_rate` (security scan: how many checked / how many unsafe / risk labels / unsafe rate), `ath_mc`, `exited`, `own_img_reuse` (logo-reuse count among the dev's OWN launches = reskin signal, computed from created-tokens), `cto`. Live = `token info` (creator/exited) + `portfolio created-tokens` (launch history, per-token classification by `_merge_created`) + `token security` ×N (`_scan_dev_security`); Mock synthesizes an isomorphic one. `_dev_reskin(dp)` derives reskin strength from `own_img_reuse`; `_security_unsafe(sec, chain)` classifies a single token's safety per chain.
